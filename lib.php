...
function theme_citricityxund_update_settings_images($settingname) {                                                                         
    global $CFG;                                                                                                                    
                                                                                                                                    
    // The setting name that was updated comes as a string like 's_theme_citricityxund_loginbackgroundimage'.                               
    ...
    if ($file = $fs->get_file_by_hash(sha1($fullpath))) {                                                                           
        ...
        $pathname = $CFG->dataroot . '/pix_plugins/theme/citricityxund/' . $settingname . '.' . $extension;                                 
                                                                                                                                    
        // This pattern matches any previous files with maybe different file extensions.                                            
        $pathpattern = $CFG->dataroot . '/pix_plugins/theme/citricityxund/' . $settingname . '.*';                                          
                                                                                                                                    
        // Make sure this dir exists.                                                                                               
        @mkdir($CFG->dataroot . '/pix_plugins/theme/citricityxund/', $CFG->directorypermissions, true);                                      
